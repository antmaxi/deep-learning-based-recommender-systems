...
sv = tf.train.Supervisor(logdir=config.logdir, save_model_secs=60 * 10, save_summaries_secs=0)
...
# If we start from sractch load pretrained model embeddings user/item memory m_u and e_i
if not FLAGS.resume:
...
for i in range(FLAGS.iters):
    ...
    loss=[]
    # Old
    # progress=tqdm(enumerate(dataset.get_data(FLAGS.batch_size, True, FLAGS.neg)), dynamic_ncols=True, total=(dataset.train_size * FLAGS.neg) // FLAGS.batch_size)
    batch=dataset.get_data(FLAGS.batch_size, True, FLAGS.neg)
    # Old for k, example in progress:
...
with open("{}/ratings_matrix".format(config.logdir), 'w') as f:
    header="user, item1:rating1,...,item99:rating99, trueItem:rating"
    f.write("{}\n".format(header))
    f.write(out)
