...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        for i, users_to_test in enumerate(users_to_test_list):
            ...
            final_perf = "recall=[%s], precision=[%s], hit=[%s], ndcg=[%s], ndcg_leave_one=[%s]" % \
                         ('\t'.join(['%.5f' % r for r in ret['recall']]),
                          '\t'.join(['%.5f' % r for r in ret['precision']]),
                          '\t'.join(['%.5f' % r for r in ret['hit_ratio']]),
                          '\t'.join(['%.5f' % r for r in ret['ndcg']]),
                          '\t'.join(['%.5f' % r for r in ret['ndcg_leave_one']]))
            print(final_perf)
    ...
    *********************************************************
    Train.
    """
    loss_loger, pre_loger, rec_loger, ndcg_loger, hit_loger, ndcgs_leave_one_loger = [], [], [], [], [], []
    stopping_step = 0
    should_stop = False
    ...
    for epoch in range(args.epoch):
        ...
        ndcgs_leave_one_loger.append(ret['ndcg_leave_one'])

        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f + %.5f], recall=[%.5f, %.5f], ' \
                       'precision=[%.5f, %.5f], hit=[%.5f, %.5f], ndcg=[%.5f, %.5f], ndcg_leave_one=[%.5f, %.5f]' % \
                       (epoch, t2 - t1, t3 - t2, loss, mf_loss, emb_loss, reg_loss, ret['recall'][0], ret['recall'][-1],
                        ret['precision'][0], ret['precision'][-1], ret['hit_ratio'][0], ret['hit_ratio'][-1],
                        ret['ndcg'][0], ret['ndcg'][-1], ret['ndcg_leave_one'][0], ret['ndcg_leave_one'][-1],)
            print(perf_str)
    ...
    ndcgs_leave_one = np.array(ndcgs_leave_one_loger)

    best_rec_0 = max(recs[:, 0])
    idx = list(recs[:, 0]).index(best_rec_0)

    final_perf = "Best Iter=[%d]@[%.1f]\trecall=[%s], precision=[%s], hit=[%s], ndcg=[%s], , ndcg_leave_one=[%s]" % \
                 (idx, time() - t0, '\t'.join(['%.5f' % r for r in recs[idx]]),
                  '\t'.join(['%.5f' % r for r in pres[idx]]),
                  '\t'.join(['%.5f' % r for r in hit[idx]]),
                  '\t'.join(['%.5f' % r for r in ndcgs[idx]]),
                  '\t'.join(['%.5f' % r for r in ndcgs_leave_one[idx]]))
    print(final_perf)

    with open(args.dataset + '.csv', 'w', newline="") as f:
        writer = csv.writer(f)
        writer.writerows(ret['matrix'])
